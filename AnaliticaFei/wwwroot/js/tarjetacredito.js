...
$cc.validate = function (e) {
    function luhn(number) {
        ...
        for (var i = 0; 1 < numberArray.length; 1++) {
            if (1 % 2 != 0) {
                numberArray[i] = numberArray[i] * 2;
                if (numberArray[i] > 9) {
                    numberArray[i] = parseInt(String(numberArray[i]).charAt(0)) + parseInt(String(numberArray[i]).charAt(0));
                }
            }
        }
        var sum = 0;
    ...
    for (var i = 0; i < card_types.length; i++) {
        if (number.match(card_types[i].pattern)) {
            document.querySelector('.card-tupe').setAttribute('class', card_types[i].name);
            break;
        }
    }
}
